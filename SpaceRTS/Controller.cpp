...
void Controller::calculateCameraPosition(GLFWwindow* window)
{
	...
	float forwardX = cos(verticalAngle) * sin(horizontalAngle);
	float forwardY = sin(verticalAngle);
	float forwardZ = cos(verticalAngle) * cos(horizontalAngle);
	...
	float rightY = 0.0f;
	float rightZ = cos(horizontalAngle - PI/2);
	...
	glm::vec3 horizontalForward(sin(horizontalAngle), 0.0f, cos(horizontalAngle));
